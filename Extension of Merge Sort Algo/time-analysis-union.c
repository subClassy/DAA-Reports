...
{
	FILE *fp1, *fp2;
	fp1 = fopen("best-case-union.txt","w");
	fp2 = fopen("worst-case-union.txt","w");
	
	   	printf("Performing best case scenario for union of sets\n");
	    for(int i=1;i<=199;i++)
	    {
		    for(int j=1;j<=i;j++)
		    {
	   	...
	   	printf("Performing worst case scenario for union of sets\n");
	    for(int i=1;i<=100;i++)
	    {
		    for(int j=1;j<=i;j++)
		    {
